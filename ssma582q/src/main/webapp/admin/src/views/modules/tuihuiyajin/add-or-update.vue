<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="场地类型" prop="changdileixing">
          <el-input v-model="ruleForm.changdileixing" 
              placeholder="场地类型" clearable  :readonly="ro.changdileixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="场地类型" prop="changdileixing">
              <el-input v-model="ruleForm.changdileixing" 
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="退回时间" prop="tuihuishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.tuihuishijian" 
                type="datetime"
                placeholder="退回时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tuihuishijian" label="退回时间" prop="tuihuishijian">
              <el-input v-model="ruleForm.tuihuishijian" 
                placeholder="退回时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.tuihuishijian = this.getCurDateTime()
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tuihuiyajin/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tuihuiyajinCrossAddOrUpdateFlag = false;
						 this.parent.search();
